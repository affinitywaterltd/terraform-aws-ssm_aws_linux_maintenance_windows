...
resource "aws_ssm_maintenance_window_task" "default_task_updates" {
  ...
  task_invocation_parameters {
    run_command_parameters {
      ...
      parameter {
        name   = "commands"
        values = ["sudo yum update-minimal --security -y --skip-broken --exclude=kernel*,mongo*,elastic*,samba*","sudo yum update -y kernelcare"]
      }
    }
  }
}
...
resource "aws_ssm_maintenance_window_task" "default_task_email_notification" {
  ...
  task_invocation_parameters {
    run_command_parameters {
      ...
      parameter {
        name   = "commands"
        values = ["mailx -s `hostname` -r ssm_patching@affinitywater.co.uk tariq.hasan@affinitywater.co.uk, chris.saunders@affinitywater.co.uk < /var/log/yum.log"]
      }
    }
  }
}
